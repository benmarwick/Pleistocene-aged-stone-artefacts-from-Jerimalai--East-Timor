...
# this comes from http://www.indiana.edu/~kruschke/DoingBayesianDataAnalysis/Programs/
...
dataSource = c( "data" )[1]   # change this to the name of the data table input above

# Load the data:
if ( dataSource == "data" ) { # change this to the name of the data table input above
  #fileNameRoot = paste( fileNameRoot , dataSource , sep="" )
  dataFrame = data.frame(     #  to simulate an archaeological problem
    Freq = data$Freq,      # change here 
    col1  = as.factor(data[, 1]),   # and change here
    col2 = as.factor(data[, 2]))       # and change here
  y = as.numeric(dataFrame$Freq)
  x1 = as.numeric(dataFrame$col1)
  x1names = levels(dataFrame$col1)
  x2 = as.numeric(dataFrame$col2)
  x2names = levels(dataFrame$col2)
  Ncells = length(y)
  Nx1Lvl = length(unique(x1))
  Nx2Lvl = length(unique(x2))
  x1contrastList = list( contrast1 = c(0,0,1,-1) ) # custom contrasts, can be changed as necessary
  x2contrastList = list( contrast2 = c(0,1,0,-1) )   # custom contrasts, can be changed as necessary
  x1x2contrastList = list( contrast3     # custom contrasts, can be changed as necessary
