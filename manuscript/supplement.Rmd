...
ggplot(flakes_cores_weight, aes(fill = Artclas, as.factor(phase), Weight)) +
  geom_jitter(aes(colour = Artclas), size = 0.85, alpha = 0.9, shape = 1) + 
...
The code chunk below uses a Bayesian one-way ANOVA to investigate differences in chert core mass by depostitional phase. 
...
ggplot(flakes_cores_cortex, aes( fill = Artclas, as.factor(phase), Cortex)) +
  geom_jitter(aes(colour = Artclas), alpha = 0.9, shape = 1) + 
  geom_boxplot(alpha = 0.5) +
  scale_y_log10() +
  xlab('depostional phase') +
  ylab("percent cortex") +
...
The code chunk below uses a Bayesian one-way ANOVA to investigate differences in chert flake cortex by depostitional phase. 
...
# subset summaryInfo to show HDI interval for interactions
HDI_intervals_for_interactions <- summaryInfo_ANOVA[14:17, c("HDIlow", "HDIhigh")]
kable(round(HDI_intervals_for_interactions,3), caption = "Highest density intervals (95%) for the posterior distributions of the interactions between phases and flake cortex")
```
