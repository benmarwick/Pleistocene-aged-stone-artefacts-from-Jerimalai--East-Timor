...
HDI_intervals_for_interactions <- summaryInfo[321:324, c("HDIlow", "HDIhigh")]
kable(HDI_intervals_for_interactions, caption = "Highest density intervals (95%) for the posterior distributions of the interactions between phases and raw material frequencies. All HDIs include zero indicating that the phases are practically equivalent in the frequencies of raw materials")
```

The code chunk above indicated no credible difference in raw material frequences over time.
...
# save plot
jhe_190mm_ggsave("figures/fig_4_Jeremalai-flake-discard-phase-m3.png")
```

# Results: Artefact taphonomy

The code chunk below generates a plot of frequencies of complete flakes, transversely broken flakes and longitudinally broken flakes made from chert in each depositional unit at Jerimalai square B.
...
myDataFrame = data
yName=names(myDataFrame)[1]   # break type
x1Name=names(myDataFrame)[2]  # phase
x2Name=names(myDataFrame)[3]  # Freq
x1contrasts = list( 
  list( c("phase 1") , c("phase 2") , compVal=0.0 , ROPE=c(-0.1,0.1) ) ,
...
# subset summaryInfo to show HDI interval for interactions
HDI_intervals_for_interactions <- summaryInfo[236:239, c("HDIlow", "HDIhigh")]
kable(HDI_intervals_for_interactions, caption = "Highest density intervals (95%) for the posterior distributions of the interactions between phases and flake breakage classes. All HDIs include zero indicating that the phases are practically equivalent in the frequencies of breakage classes")
```

The above code chunk returns results that indicate not credible difference in the frequences of flake breakage types by phase. 
