...
The Bayesian test includes zero in the HDI, indicating a non-credible difference. The t-test In the code chunk above we obtain the results of a frequentist t-test which returns a statistically significant result: t = `r unname(sq_A_raw_material_by_phase_nhst[[1]])`, df = `r unname(sq_A_raw_material_by_phase_nhst[[2]])`, p = `r unname(sq_A_raw_material_by_phase_nhst[[3]])`. This shows a statistically significant change in flake raw material, however the low Cramer's V indicates a very small effect size. This is consistant with what we see in the Bayesian HDI intervals, and we interpret these results as an insubstantial increase in the very small proportion of artefacts made from volcanic rock. 
...
```{r sq_a_chert_flake_cortex_test_bayes, eval=FALSE}
